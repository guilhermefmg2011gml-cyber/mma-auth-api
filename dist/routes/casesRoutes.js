...
router.get("/tavily", async (req, res) => {
    try {
        const rawName = (typeof req.query.nome === "string" && req.query.nome.trim()) ||
...
router.get("/", async (_req, res) => {
    try {
        const rows = db
            .prepare(`SELECT id, numero_cnj, tribunal, orgao, classe, assunto, atualizado_em FROM cases ORDER BY atualizado_em DESC`)
            .all();
        res.json(rows);
    }
    catch (error) {
