...
export { TIPOS_PECA };
function recordLocalMemoryEntry(texto, tipo, { clienteId, processoId, }) {
...
function mapSectionsToTemplate(texto, template) {
...
async function ensureIntelligentPedidos(texto, input, template) {
    ...
    const fundamentacaoBlocks = template.blocos.filter((bloco) => /fundament|teses_defendidas|fundamento_tecnico|reforma_pleiteada/i.test(bloco));
    ...
    for (const bloco of blocksComPedidos) {
        ...
        try {
            ...
            console.warn(`[legalDocGenerator] falha ao gerar pedidos inteligentes para ${bloco}`, error);
        }
    }
...
function extractLawArticles(texto) {
    if (!texto) {
        return [];
    }
...
async function verifyLawArticles(artigos) {
    if (!artigos.length) {
        return [];
    }
    const verificacoes = artigos.map(async (artigo) => {
        try {
            ...
            const resultados = await searchLegalInsights(query, ARTICLE_VERIFICATION_DOMAINS, 4);
...
function annotateArticlesInText(texto, validacoes) {
...
function buildDocumentXml(piece) {
    ...
    const institutionalHeader = [
        `<w:p><w:pPr><w:jc w:val="center"/></w:pPr><w:r><w:rPr><w:b/><w:sz w:val="28"/><w:szCs w:val="28"/></w:rPr><w:t>${escapeXml("MOURA MARTINS ADVOGADOS")}</w:t></w:r></w:p>`,
        `<w:p><w:pPr><w:jc w:val="center"/></w:pPr><w:r><w:rPr><w:sz w:val="20"/><w:szCs w:val="20"/></w:rPr><w:t>${escapeXml("OAB/GO • mouramartinsadvogados.com.br")}</w:t></w:r></w:p>`,
...
function buildCorePropsXml(piece) {
    const titulo = `Peça ${formatTitle(piece.tipo.replace(/_/g, " "))}`;
...
export async function buildDocxFromPiece(piece) {
