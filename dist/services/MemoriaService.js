...
export class MemoriaService {
    static client = new ChromaClient({ path: 'https://api.trychroma.com' });
    static async getCollection() {
        return this.client.getOrCreateCollection({ name: COLLECTION_NAME });
    }
    static async salvar({ texto, vetor, clienteId, processoId }) {
        if (!texto?.trim() || !Array.isArray(vetor) || !vetor.length) {
            return;
        }
        const metadados = {};
        if (clienteId)
            metadados.clienteId = clienteId;
        if (processoId)
            metadados.processoId = processoId;
    ...
    static async buscar({ consulta, vetor, clienteId, processoId, topK = 5 }) {
