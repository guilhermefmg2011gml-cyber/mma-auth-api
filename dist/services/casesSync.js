 ...
 */
function inferMetaFromResult(result, _cnj) {
    const url = typeof result.url === "string" ? result.url : "";
    const title = typeof result.title === "string" ? result.title : "";
    const content = typeof result.content === "string" ? result.content : "";
    // Tribunal básico pelo domínio
    let tribunal = "";
    if (url.includes("tjgo.jus.br") || title.includes("TJGO"))
        tribunal = "TJGO";
    if (url.includes("stj.jus.br") || title.includes("STJ"))
        tribunal = "STJ";
    if (url.includes("tst.jus.br") || title.includes("TST"))
        tribunal = "TST";
    if (url.includes("trf") || title.includes("TRF"))
        tribunal = "TRF";
    // Classe / assunto: heurística simples inicial
    const textoBase = `${title} ${content}`.toUpperCase();
    let classe = "";
    if (textoBase.includes("MANDADO DE SEGURANÇA"))
        classe = "Mandado de Segurança";
    else if (textoBase.includes("APELAÇÃO"))
        classe = "Apelação";
    else if (textoBase.includes("AGRAVO"))
        classe = "Agravo";
...
function upsertCase(record) {
    ...
    if (existing && record.tribunal && record.tribunal !== existing.tribunal) {
        db.prepare("UPDATE cases SET tribunal = ?, atualizado_em = ? WHERE id = ?").run(record.tribunal, now, existing.id);
...
export async function syncCasesFromTavily() {
    ...
    tx();
    console.log(`[syncCasesFromTavily] Total CNJs distintos encontrados: ${total}, novos inseridos: ${inserted}`);
    return {
        ok: true,
        found: total,
        inserted,
